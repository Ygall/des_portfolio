# modules/mod_connaissances.R — Portfolio de connaissances (bulk edit)
# ─────────────────────────────────────────────────────────────────────────────
      ...
      column(8,
        box(title = uiOutput(ns("tbl_title")), width = NULL,
            ...
            div(style = "font-size:0.82em; color:#666; margin-bottom:8px;",
                ...
                " pour sélectionner plusieurs lignes simultanément.",
...
mod_connaissances_server <- function(id, user_id_r) {
  moduleServer(id, function(input, output, session) {
    ...
    # Titre dynamique table
    output$tbl_title <- renderUI({
      n <- n_sel()
      if (n == 0) "Référentiel de connaissances"
      else span("Référentiel de connaissances —",
                span(style = "color:#f39c12; font-weight:700;",
                     paste(n, "item(s) sélectionné(s)")))
    })

    # Titre dynamique panneau
      ...
      if (nrow(d) == 0)
        return(datatable(
          data.frame(Domaine=character(), Niveau=character(),
                     Connaissance=character(), `Auto-éval`=character(),
                     `Val. Senior`=character(), check.names=FALSE),
      ...
      datatable(display,
        ...
        options    = list(
          ...
          language   = list(
            ...
            select  = list(rows = list(`1` = "1 item sélectionné",
    ...
    output$panel_bulk <- renderUI({
      ...
      d    <- filtered_data()
      # Pré-remplissage si 1 seul item
      ...
      tagList(
        # Indicateur visuel du mode bulk
        if (n > 1) div(
          class = "alert alert-info",
          style = "padding:6px 10px; font-size:0.85em; margin-bottom:10px;",
          icon("bolt"),
          sprintf(" Les valeurs remplies seront appliquées aux %d items sélectionnés.", n),
          br(),
          "Les champs laissés vides ", strong("ne seront pas modifiés"), "."
        ),

        div(style = "margin-bottom:8px; font-size:0.82em; color:#888;",
            if (n == 1 && !is.null(item))
              em(norm_lib(item$libelle))
            else
              em(paste(n, "items sélectionnés dans le tableau"))
        ),
        ...
        # ── Champ validation senior ────────────────────────────────────────────
        div(class = "bulk-field",
          if (n > 1) checkboxInput(ns("apply_senior"),
                                   "Modifier la validation Senior", value = FALSE),
    ...
    observeEvent(input$btn_save_item, {
      ...
      # En mode bulk, seuls les champs cochés sont appliqués
      ...
      for (idx in sel) {
        ...
        tryCatch({
          upsert_eval_connaissance(
            ...
            date_eval_senior  = if (apply_dt) val_dt  else item$date_eval_senior %||% val_dt,
      ...
      if (n_err == 0)
        showNotification(
          paste0(n_ok, " item(s) enregistré(s) ✓"),
          type = "message", duration = 3)
      else
        showNotification(
          paste0(n_ok, " ok / ", n_err, " erreur(s)"),
