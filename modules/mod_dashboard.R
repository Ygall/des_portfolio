...
mod_dashboard_interne_server <- function(id, user_id_r) {
  moduleServer(id, function(input, output, session) {
    ...
    d_conn  <- reactive({ refresh_trigger(); req(user_id_r()); get_eval_connaissances(user_id_r()) })
    d_comp  <- reactive({ refresh_trigger(); req(user_id_r()); get_eval_competences(user_id_r()) })
    ...
    output$vb_stages <- renderValueBox({
      ...
      valueBox(paste0(n_r,"/",nrow(d)), "Stages renseignés",
               icon = icon("hospital"),
               color = if(n_r==nrow(d) && nrow(d)>0)"green" else "yellow")
    })

    # Helper barplot empilé avec alias
    .barplot <- function(d, cols_statut, labels_statut, colors, titre, filtre) {
      ...
      if (nrow(d) == 0) return(ggplot() + theme_void() +
        labs(title = titre) + theme(plot.title = element_text(face="bold")))

      # Compter chaque statut par domaine sans across() pour éviter l'ambiguïté
      prog <- d |>
      ...
      ggplot(prog, aes(x=reorder(alias,n), y=n, fill=s)) +
        ...
        labs(title=paste0(titre, if(!is.null(filtre)&&filtre!="tous") paste0(" [",filtre,"]") else ""),
    ...
    output$tbl_phases <- renderDT({
      ...
      df <- data.frame(
        ...
        Date  = sapply(PHASES, function(ph) {
          row <- dp[dp$phase==ph,]; if(nrow(row)==0)"—" else row$date_validation[1]%||%"—"
        }),
        Signataire = sapply(PHASES, function(ph) {
          row <- dp[dp$phase==ph,]; if(nrow(row)==0)"—" else row$validateur[1]%||%"—"
        }),
...
mod_dashboard_coord_server <- function(id, user, interne_cible) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      promos <- db_query("SELECT DISTINCT promotion FROM users WHERE role='interne' AND active=1 ORDER BY promotion")$promotion
    ...
    output$vb_n <- renderValueBox(
      valueBox(nrow(internes_data()),"Internes suivis",icon=icon("users"),color="blue"))

    output$vb_moy <- renderValueBox({
      internes <- internes_data()
      if(nrow(internes)==0) return(valueBox("—","Moy. conn. acquises",icon=icon("book"),color="yellow"))
    ...
    output$tbl_promotion <- renderDT({
      internes <- internes_data()
      if(nrow(internes)==0) return(datatable(data.frame(Message="Aucun interne"),rownames=FALSE))
      rows <- lapply(seq_len(nrow(internes)),function(i){
        ...
        data.frame(
          ...
          `% Conn.` = if(nrow(dc)==0)"0%" else paste0(round(100*sum(dc$autoeval=="acquis",na.rm=TRUE)/nrow(dc),1),"%"),
          `% Comp.` = if(nrow(dco)==0)"0%" else paste0(round(100*sum(dco$autoeval%in%c("acquis","en_cours"),na.rm=TRUE)/nrow(dco),1),"%"),
    ...
    .dist <- function(evals,col,lvls,lbls,cols,titre){
      ...
      ggplot(cnt,aes(statut,Freq,fill=statut))+geom_col(width=.55)+
    ...
    output$plot_dist_conn <- renderPlot({
      ...
      .dist(d,"autoeval",c("non_evalue","non_acquis","acquis"),
            c("Non évalué","Non acquis","Acquis"),
            c("Non évalué"="#adb5bd","Non acquis"="#e74c3c","Acquis"="#27ae60"),
    ...
    output$plot_dist_comp <- renderPlot({
      ...
      .dist(d,"autoeval",c("non_evalue","non_acquis","en_cours","acquis"),
            c("Non évalué","Non acquis","En cours","Acquis"),
            c("Non évalué"="#adb5bd","Non acquis"="#e74c3c","En cours"="#f39c12","Acquis"="#27ae60"),
